...
class AstroCLIP(L.LightningModule):
    ...
    def forward(self, x: Dict[str, Dict[str, torch.FloatTensor]], image: bool = True, return_weights: bool = False):
        # Embed image
        if image: 
    ...
    def training_step(self, batch, batch_idx):
        ...
        loss_nologit = self.criterion(image_features, spectrum_features, self.hparams.logit_scale)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        image_features = self.image_encoder((im.cuda(), None))
        spectrum_features = self.spectrum_encoder(sp)
        val_loss_nologit = self.criterion(image_features, spectrum_features, self.hparams.logit_scale)
