...
sys.path.insert(
    0,
    os.path.abspath(
        "/mnt/home/lparker/Documents/AstroFoundationModel/AstroDino/dinov2/dinov2"
    ),
)
sys.path.insert(
    0,
...
class CLIPLoss(nn.Module):
    ...
    def forward(
        self, image_features, spectrum_features, logit_scale, output_dict=False
    ):
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--embedding_dir", type=str, help="Directory to save embeddings"
    )
    parser.add_argument(
        "--batch_size", type=int, default=1024, help="Batch size for training"
    )
    parser.add_argument(
        "--embed_dim", type=int, default=128, help="Batch size for training"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=float,
        default=15.5,
        help="Softmax temperature in loss for training",
    )
    parser.add_argument(
        "--freeze_backbone",
        type=int,
        default=1,
        help="Train or freeze pre-trained embedders?",
    )
    ...
    img_model = OutputExtractor(
        model, embed_dim=embed_dim, freeze_backbone=args.freeze_backbone
    )
    ...
    spectrum_encoder = seq_decoder(
        model=spec_model, embed_dim=embed_dim, freeze_backbone=args.freeze_backbone
    )
    ...
    wandb_run = wandb.init(
        ...
        project="astroclip-clip-explore",
        config={
            "embed_dim": embed_dim,
            "model_type": "very_complicated",
            "batch_size": args.batch_size,
            "args": vars(args),
        },
    )
    wandb_logger = WandbLogger(
        project="astroclip-clip-explore", id=wandb_run.id, entity="flatiron-scipt"
    )
    ...
    train_images, train_spectra, train_targetids = (
        torch.load("/mnt/home/lparker/ceph/images_train.pt"),
        torch.load("/mnt/home/lparker/ceph/spectra_train.pt"),
        torch.load("/mnt/home/lparker/ceph/targetids_train.pt"),
    )
    val_images, val_spectra, val_targetids = (
        torch.load("/mnt/home/lparker/ceph/images_val.pt"),
        torch.load("/mnt/home/lparker/ceph/spectra_val.pt"),
        torch.load("/mnt/home/lparker/ceph/targetids_val.pt"),
    )
