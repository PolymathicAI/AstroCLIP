...
def get_data(embedding_file, images=True):
    ...
    for i, p in enumerate(properties):
        ...
        if p == 'SFR': prop_train, prop_test = np.log(prop_train)-train_provabgs['LOG_MSTAR'].reshape(-1, 1), np.log(prop_test)-test_provabgs['LOG_MSTAR'].reshape(-1, 1)
...
def main(embedding_file, save_dir, source='images', train_type='zero_shot', stein_embedding_file='/mnt/home/lparker/ceph/stein_propertyembeddings.h5', DINO_embedding_file='/mnt/home/lparker/ceph/DINO_embeddings.h5', GalFormer_embedding_file='/mnt/home/lparker/ceph/GalFormer_embeddings.h5'):
    # Create Directory
    ...
    if source == 'images':
        ...
        if train_type == 'zero_shot': 
            r2_scores['CLIP'] = zero_shot_train(data['X_train'], data['y_train'], data['X_test'], data['y_test'], properties)
            r2_scores['Stein et. al.'] = zero_shot_train(stein_data['X_train'], stein_data['y_train'], stein_data['X_test'], stein_data['y_test'], properties)
            r2_scores['DINO'] = zero_shot_train(DINO_data['X_train'], DINO_data['y_train'], DINO_data['X_test'], DINO_data['y_test'], properties)
            
        # Do Few-Shot with MLP
        elif train_type == 'few_shot':             
            r2_scores['CLIP'] = few_shot_train(data['X_train'], data['y_train'], data['X_test'], data['y_test'], properties)
            r2_scores['Stein et. al.'] = few_shot_train(stein_data['X_train'], stein_data['y_train'], stein_data['X_test'], stein_data['y_test'], properties)
            r2_scores['DINO'] = few_shot_train(DINO_data['X_train'], DINO_data['y_train'], DINO_data['X_test'], DINO_data['y_test'], properties)
            
        else: raise ValueError('Only train types are zero_shot or few_shot')
                
        # Get Baselines from ResNet18
        resnet18 = torch.load('./baseline_models/resnet_results', map_location=torch.device(device))
        r2_scores['ResNet18'] = resnet_r2(resnet18, properties)
        
        # Get Baselines from Photometry
        photo_mlp = torch.load('./baseline_models/photometry_results', map_location=torch.device(device))
        ...
        if train_type == 'zero_shot':
            r2_scores['CLIP'] = zero_shot_train(data['X_train'], data['y_train'], data['X_test'], data['y_test'], properties)
            r2_scores['Transformer'] = zero_shot_train(GalFormer_data['X_train'], GalFormer_data['y_train'], GalFormer_data['X_test'], GalFormer_data['y_test'], properties)

        if train_type == 'few_shot':
            r2_scores['CLIP'] = few_shot_train(data['X_train'], data['y_train'], data['X_test'], data['y_test'], properties)
            r2_scores['Transformer'] = few_shot_train(GalFormer_data['X_train'], GalFormer_data['y_train'], GalFormer_data['X_test'], GalFormer_data['y_test'], properties)
            
        # Get Baseline from Spender
        spender = torch.load('./baseline_models/spender_results', map_location=torch.device(device))
    ...
    else: raise ValueError('Only accepting source of images or spectra')
    
    
if __name__=='__main__':
    parser = argparse.ArgumentParser(description="Zero- or few- shot learning on images or spectra embeddings")
    parser.add_argument('--embedding_file', type=str, default='/mnt/home/lparker/ceph/newest_embeddings.h5', help='Path to the embedding file.')
    ...
    main(args.embedding_file, args.save_dir, source=args.source, train_type=args.train_type)
