...
class ConditionalFlowStack(dist.conditional.ConditionalComposeTransformModule):
    def __init__(self, input_dim, context_dim, hidden_dims, num_flows):
        ...
        coupling_transforms = [
            T.conditional_spline(input_dim, context_dim, count_bins=4, hidden_dims=hidden_dims, order='linear').cuda()
...
def do_flow(data, num_ndes, n_samples=500, max_hidden=128, max_flows=4, min_flows=None, num_epochs=100, lr=5e-5, batch_size=256):
    if min_flows == None:
        if max_flows < 3: min_flows = 1
        else: min_flows = max_flows - 2
    ...
    for i in range(num_ndes):
        nhidden = int(np.ceil(np.exp(np.random.uniform(np.log(max_hidden/2), np.log(max_hidden)))))
        nblocks = int(np.random.uniform(min_flows, max_flows))

        print('Flow %i with nhidden=%i; ncomponents=%i:' % (i, nhidden, nblocks))

        transform = ConditionalFlowStack(input_dim, context_dim, [nhidden, nhidden], nblocks).to('cuda')
        try:
...
flow_results = {'Flows': {}, 'Samples': {}, 'NLL': {}, 'Average NLL': {}, 'Variance NLL': {}}
print('Training and evaluating flows...\n')

hyperparams = {'images': {'max_hidden': 32, 'max_flows': 3, 'min_flows': 2, 'lr': 5e-4},
...
for key in datasets.keys():
    ...
    with open(f'/mnt/home/lparker/Documents/AstroFoundationModel/AstroBaselines/property_estimation/flows/new_{key}_flows_nll.txt', 'w') as f:
        for subkey in source_datasets.keys():
            data = source_datasets[subkey]
            max_hidden, max_flows, min_flows, lr = hyperparams[key]['max_hidden'], hyperparams[key]['max_flows'], hyperparams[key]['min_flows'], hyperparams[key]['lr']            
            flow, nll, samples = do_flow(data, num_ndes=5, n_samples=500, max_hidden=max_hidden, max_flows=max_flows, min_flows=min_flows, num_epochs=200, lr=lr)
            ...
            torch.save(flow, f'/mnt/home/lparker/Documents/AstroFoundationModel/AstroBaselines/property_estimation/flows/{key}/new_{subkey}_flow.pt')
            torch.save(samples, f'/mnt/home/lparker/Documents/AstroFoundationModel/AstroBaselines/property_estimation/flows/{key}/new_{subkey}_samples.pt')
            print('')
