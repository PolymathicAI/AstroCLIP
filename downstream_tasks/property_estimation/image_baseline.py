...
def get_provabgs(provabgs_file, cache_dir):
    dataset = load_dataset('/mnt/home/lparker/Documents/AstroFoundationModel/AstroBaselines/datasets/legacy_survey.py', cache_dir=cache_dir)
    dataset.set_format(type='torch', columns=['image', 'redshift', 'targetid'])
        
    train_table  = Table({'targetid': np.array(dataset['train']['targetid']), 'image': np.array(dataset['train']['image'])})
    test_table  = Table({'targetid': np.array(dataset['test']['targetid']), 'image': np.array(dataset['test']['image'])})    
...
def train_CNN(model, train_loader, val_loader, test_loader, prop_scalers, device='cuda', num_epochs=50):
...
def main(provabgs_file, cache_dir, save_path):
    ...
    test_r2s, model, train_preds, train_trues, test_preds, test_trues, scalers = train_CNN(model, train_loader, val_loader, test_loader, prop_scalers)

    # Save Results
    resnet_results = {'r2': test_r2s, 'model': model, 'test_trues': test_trues, 'test_preds': test_preds, 'train_trues': train_trues, 'train_preds': train_preds, 'scalers': scalers}
...
if __name__ == '__main__':
    ...
    parser.add_argument('--save_path', type=str, default='./baseline_models/resnet_results', help='Path to Saved ResNet')
