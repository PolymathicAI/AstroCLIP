...
def get_data(provabgs_file, cache_dir, batch_size=256):
    ...
    X_train = torch.tensor(np.stack([train_dataset['MAG_G'], train_dataset['MAG_R'], train_dataset['MAG_Z']]), dtype=torch.float32).permute(1, 0)
    X_test = torch.tensor(np.stack([test_dataset['MAG_G'], test_dataset['MAG_R'], test_dataset['MAG_Z']]), dtype=torch.float32).permute(1, 0)
...
def main(provabgs_file, cache_dir, save_file):
    ...
    # R2 Calculation
    r2s, preds = few_shot_train(model, X_train, y_train, X_test, y_test, properties, r2_only=False)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="Photometry MLP Baseline for Galaxy Property Estimation")
    parser.add_argument('--save_path', type=str, help="Model output path")
    parser.add_argument('--cache_dir', type=str, default="/mnt/ceph/users/lparker/datasets_astroclip")
    parser.add_argument('--provabgs_file', type=str, default='/mnt/home/lparker/ceph/BGS_ANY_full.provabgs.sv3.v0.hdf5')
