...
def main(embedding_file, save_dir, source="images", train_type="zero_shot"):
    if source == "images":
        ...
        raise ValueError("Only accepts images or spectra")
    ...
    if train_type == "zero_shot":
        ...
        raise ValueError("Only accepts zero_shot or few_shot")

    truth = data["y_test"]

    fig, axes = plt.subplots(
        2, len(properties), figsize=(30, 10)
    )  # 5 subplots, 1 row, 25x5 figure size
    plt.subplots_adjust(
    ...
    for ax, (property, config) in zip(axes[0], prop_dict.items()):
        ...
        sns.histplot(
            ax=ax, x=true_data, y=preds_data, bins=64, pthresh=0.1, cmap="mako"
        )
        ...
        ax.text(
            0.9,
            0.1,
            "$R^2$ score: %0.2f" % r2_score(true_data, preds_data),
    ...
    for ax, (property, config) in zip(axes[1], prop_dict.items()):
        idx = properties.index(property)
        true_data = (
            baseline["scalers"][property]
            .inverse_transform(baseline_true[property].reshape(-1, 1))
            .squeeze()
        )
        preds_data = (
            baseline["scalers"][property]
            .inverse_transform(baseline_pred[property].reshape(-1, 1))
            .squeeze()
        )
    ...
    if axes[1].size > 0:
        axes[1][0].set_ylabel(
            f"Supervised {baseline_type}", fontsize=25
        )  # pad for some spacing

    plt.tight_layout(
        rect=[0, 0, 1, 0.97]
    )  # Adjust the rect so the main title is visible and not overlapping
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--embedding_file",
        type=str,
        default="/mnt/home/lparker/ceph/property_embeddings.h5",
