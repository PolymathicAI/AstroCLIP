    ...
    train_table = Table(
        {
            ...
            "image": np.array(dataset["train"]["spectrum"]),
        }
    )
    test_table = Table(
        {
            "targetid": np.array(dataset["test"]["targetid"]),
            "image": np.array(dataset["test"]["spectrum"]),
        }
    )
...
def main(provabgs_file, cache_dir, save_path):
    ...
    test_r2s, model, train_pred, train_true, test_pred, test_true, scalers = train_CNN(
        model, train_loader, val_loader, test_loader, prop_scalers
    )
    ...
    spender_results = {
        ...
        "train_trues": train_true,
        "test_preds": test_pred,
        "test_trues": test_true,
        "scalers": scalers,
    }
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--save_path",
        type=str,
        default="./baseline_models/spender_results_testing2",
        help="Path to Saved Spender",
    )
