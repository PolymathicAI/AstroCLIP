...
def plot_radar(
    ...
    for key, array in data_dict.items():
        if key != label_key and len(array) != num_vars:
            raise ValueError(
                f"All arrays must have the same length as the label array."
            )
...
def gaussian_kernel(size, sigma):
    ...
    grid = np.mgrid[
        size // 2 - size + 1 : size // 2 + 1, size // 2 - size + 1 : size // 2 + 1
    ]
...
def resnet_augmentations(images, kernel_size=5):
    ...
    for i in range(images.size(0)):
        ...
        blurred_images[i] = F.conv2d(
            images[i].unsqueeze(0), blur_kernel, padding=padding, groups=3
        ).squeeze(0)
